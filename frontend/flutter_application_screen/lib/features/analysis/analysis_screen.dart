...
class _AnalysisScreenState extends State<AnalysisScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // .env 縺九ｉ GeminiService 繧貞・蛻晄悄蛹・(Dashboard縺ｧ蛻晄悄蛹匁ｸ医∩縺縺後∫判髱｢驕ｷ遘ｻ蠕後ｂ菴ｿ縺医ｋ繧医≧縺ｫ)
      ...
      widget.analysisFuture!.then((result) {
        ...
        if (mounted) {
          ...
          if (e is AppException) {
            ...
            errorMsg = isDebug ? 'error:$e' : 'error:隗｣譫舌↓螟ｱ謨励＠縺ｾ縺励◆縲ゅ＠縺ｰ繧峨￥蠕・▲縺ｦ縺九ｉ繧・ｊ逶ｴ縺励※縺上□縺輔＞縲・;
          }
          // 繧ｨ繝ｩ繝ｼ譎ゅ・繝｡繝・そ繝ｼ繧ｸ繧呈ｷｻ縺医※繝繝・す繝･繝懊・繝峨↓謌ｻ繧・
  ...
  Future<void> _playAudio() async {
    if (_data.audioBytes != null) {
      try {
        ...
        // 蜀咲函繧ｨ繝ｩ繝ｼ譎ゅ・繝輔か繝ｼ繝ｫ繝舌ャ繧ｯ縺ｨ縺励※3遘貞ｾ後↓謌ｻ繧・
        _startAutoExitTimer(const Duration(seconds: 3));
      }
    } else {
      // TTS縺後↑縺・ｴ蜷医・縲∬｡ｨ遉ｺ螳御ｺ・°繧・遘貞ｾ後↓謌ｻ繧・
      _startAutoExitTimer(const Duration(seconds: 3));
    }
  }

  Future<void> _startAutoScrolling() async {
    // 繝ｦ繝ｼ繧ｶ繝ｼ謫堺ｽ懊ｒ繝ｪ繧ｻ繝・ヨ
    _isUserScrolling = false;

    // 繝ｬ繧､繧｢繧ｦ繝医′遒ｺ螳壹＠縺ｦ髻ｳ螢ｰ諠・ｱ縺悟叙蠕励〒縺阪ｋ縺ｾ縺ｧ蟆代＠蠕・ｩ・
    ...
    if (duration != null && duration.inSeconds > 0) {
      // 髻ｳ螢ｰ縺ｮ髟ｷ縺輔↓蜷医ｏ縺帙※遲蛾溘〒譛蠕後∪縺ｧ繧ｹ繧ｯ繝ｭ繝ｼ繝ｫ
  ...
  Future<void> _startVoiceIntentDetection() async {
    ...
    try {
      if (await _recorder.hasPermission()) {
        ...
        // 骭ｲ髻ｳ髢句ｧ九・蜷亥峙縺ｨ縺励※繧ｷ繧ｹ繝・Β騾夂衍髻ｳ繧貞・逕・
        SoundService.playVoiceStart();

        // VAD (Voice Activity Detection): 螢ｰ縺碁泌・繧後◆繧芽・蜍募●豁｢
        ...
        _amplitudeSub = _recorder.onAmplitudeChanged(const Duration(milliseconds: 50)).listen((amp) {
          if (mounted) {
            ...
            // 縺励″縺・､ -40dB 莉･荳翫・蜈･蜉帙′縺ゅｌ縺ｰ縲悟｣ｰ縲阪→縺ｿ縺ｪ縺・
            if (amp.current > -40) {
              lastSoundTime = DateTime.now();
            } else {
              // 1.5遘剃ｻ･荳頑ｲ磯ｻ吶′邯壹＞縺溘ｉ縲∫匱隧ｱ邨ゆｺ・→縺ｿ縺ｪ縺励※蛛懈ｭ｢
        ...
        // 譛螟ｧ10遘偵・繧ｿ繧､繝繧｢繧ｦ繝医ｒ險ｭ螳夲ｼ郁ｪｰ繧りｩｱ縺輔↑縺・ｴ蜷医↑縺ｩ・・
