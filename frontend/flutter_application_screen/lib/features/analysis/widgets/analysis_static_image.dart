...
class AnalysisStaticImage extends StatelessWidget {
  ...
  Widget _buildStaticImageArea() {
    ...
    if (polygon != null && polygon.length >= 4) {
      ...
      if (h > w * 1.2) {
        aspectRatio = 3 / 4; // 邵ｦ髟ｷ
      } else if (w > h * 1.2) {
        aspectRatio = 16 / 9; // 讓ｪ髟ｷ
      } else {
        aspectRatio = 1.0; // 豁｣譁ｹ蠖｢縺ｫ霑代＞
      }
    }
                  ...
                  child: Stack(
                    ...
                    children: [
                      ...
                      Positioned(
                        bottom: 16,
                        left: 16,
                        child: _buildBadge("笳・ANALYZING...", Colors.black.withValues(alpha: 0.6), const Color(0xFFE2F063)),
                      ),
                    ],
                  ),
                ),
                  ...
                  child: Stack(
                    ...
                    children: [
                      ...
                      Positioned(
                        bottom: 16,
                        left: 16,
                        child: _buildBadge("笳・LIVE RECOGNITION", Colors.transparent, const Color(0xFFE2F063)),
                      ),
                    ],
                  ),
                ),
  ...
  Widget _buildImageWithCrop() {
    ...
    // 蠅・阜遏ｩ蠖｢縺ｮ險育ｮ・(Gemini縺ｮ蠎ｧ讓吶・ 0~1000 縺ｮ遽・峇 [ymin, xmin, ymax, xmax] 縺ｪ縺ｩ)
    double minX = 1000, minY = 1000, maxX = 0, maxY = 0;
    for (int i = 0; i < polygon.length - 1; i += 2) {
      final y = polygon[i];
      final x = polygon[i + 1];
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        // 蟇ｾ雎｡迚ｩ縺檎判髱｢縺ｮ邏・5%繧貞頃繧√ｋ繧医≧縺ｫ繧ｺ繝ｼ繝邇・ｒ險育ｮ暦ｼ亥､ｧ縺阪☆縺弱・蟆上＆縺吶℃繧帝亟縺舌◆繧・1.0縲・.0 蛟阪↓蛻ｶ髯撰ｼ・
